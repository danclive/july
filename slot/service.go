...
func InitService(engine *xorm.Engine, readInterval int, keepalive int) {
	...
	initCache()
	initCollect(readInterval, keepalive)
}
...
func (s *Service) ListSlotByStatusOn() ([]Slot, error) {
	items := make([]Slot, 0)
...
func (s *Service) ListTagByStatusOnAndIO(slotId string) ([]Tag, error) {
	items := make([]Tag, 0)

	err := s.Where("slot_id = ?", slotId).And("tag_type = ?", TagTypeIO).And("status = ?", consts.ON).Find(&items)
	if err != nil {
		return nil, err
	}
...
func (s *Service) SlotReset(driver string) error {
	...
	if driver != "" {
		err = s.Where("driver = ?", driver).Find(&items)
	} else {
		err = s.Find(&items)
	}
